...
This was the critical insight from v2 to v3. The pass rate jumped from 79.5% to 93.2%, and the obvious interpretation was that v3 was a large improvement. But the eval had been calibrated between those two runs. I changed key_facts to check for "any" not "all" and the search strategy judge to factor in whether an answer is systematically out_of_reach. After calibrating portions of the evaluation suite, I became concerned that improvements might simply be artifacts of measurement changes rather than genuine prompt improvements so I ran a v4 eval (v2 prompt on the calibrated eval) to answer "How much of the jump was the prompt and how much was the eval getting more lenient?". V4 scored 77.3% which is nearly identical to v2's 79.5% and thus calibration had zero net effect. The pairwise comparison gave me the same answer independently. v3 winning 10-8 overall and 10-6 on groundedness, confirming a genuine prompt improvement without needing the control run.
